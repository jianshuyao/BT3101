...
def read_data(user):
    ...
    trade_table['Timestamp'] = pd.to_datetime(trade_table.Timestamp)
...
@app.callback(
    ...
    [State('portfolio', 'value'),
     ...
     State('user', 'value')])
def update_table(submit_n_clicks, sort_by, portfolio, type, product, direction, price, size, tenor, risk, timeframe, strategy, timestamp, user):
    if len(sort_by):
        trade_df = trade_table.sort_values(
