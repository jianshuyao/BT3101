...
# tab 2 check input integrity & update table
@app.callback(
    ...
    State('user', 'value')])
def check_input(submit_n_clicks, portfolio, type, product, direction, price, size, tenor, risk, timeframe, strategy, timestamp, user):
    if submit_n_clicks:
        inputs = [portfolio, type, product, direction, price, size, tenor, risk, timeframe, strategy, timestamp, user]
        for input in inputs:
            if input == None:
               return True, trade_table.to_dict('records')
    ...
    return False, trade_table.to_dict('records')   
...
def update_tab4_aggregated_view(time):
    temp_df = trade_table[trade_table['Time Frame']==time].groupby('Timestamp')['Size/Notional'].sum().to_frame().reset_index()
